...
func main() {
	...
	countedWords := make(map[string]int, 1_000)
	for _, v := range words {
		//add 1 to the word key-value pair. if that word isn't encountered yet, then create a key-value pair with a val of 1
		//when we do cW[v] and v is a string we haven't encountered, the return as we say is the zero value of the key's type. in our case it is 0 and then we ++ so it goes from 0 --> 1
		countedWords[v]++
	}
